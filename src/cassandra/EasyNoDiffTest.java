...
//    public static String[] cfs = new String[]{"normal1","normal1","normal1"}; // 最普通的无优化的
    ...
    public EasyNoDiffTest(Unify unify) {
        this.unify = unify;
        this.X = unify.X;
        Xsqls = new ArrayList();
            ...
            for(int j =0; j < unify.qchooseX.size(); j++) { // 遍历每一个查询语句
                ...
                for(int z = 0 ; z<chooseNum;z++) {
                    if(chooseX.get(z) == i) { // 都是从0开始
                        Xsqls.get(i).add(String.format(unify.sqls.get(j),ks,cfs[i],1));
                        Xqperc.get(i).add((double)unify.queriesPerc.get(j)/chooseNum);
                        break; // chooseX里不会重复的
                    }
                }
            }
        ...
        for(int i=0;i<X;i++) {
            ...
            List<String> sqls = Xsqls.get(i);
            List<Double> qperc = Xqperc.get(i);
            for(int j=0;j<sqls.size();j++) {
                System.out.printf(sqls.get(j)+":");
